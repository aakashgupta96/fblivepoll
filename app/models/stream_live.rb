class StreamLive
  ...
  def self.perform(post_id)
    ...
    if Rails.env.production?
      ...
      if source_live
        command = "$HOME/bin/ffmpeg -i '#{@post.get_file_url}' -codec:a aac -ac 1 -ar 44100 -b:a 128k -preset ultrafast -vcodec libx264 -pix_fmt yuv420p -vb 2000k -r 24 -g 48 -f tee -map 0:v 1:a '[f=flv\:onfail=ignore]#{@post.key}' 2> #{Rails.root.join('log').join('stream').join(@post.id.to_s).to_s}"
      else
        command = "$HOME/bin/ffmpeg -s 1280x720 -r 24 -f x11grab -i :#{headless.display}.0+0,66 -i 'public/silent.aac' -ac 1 -codec:a aac -ar 44100 -b:a 128k -preset ultrafast -vcodec libx264 -pix_fmt yuv420p -color_range 2 -vb 2000k -r 24 -g 48 -f tee -map 0:v 1:a '[f=flv\:onfail=ignore]#{@post.key.split(':80').join}' 2> #{Rails.root.join('log').join('stream').join(@post.id.to_s).to_s}"
      end
    end
    start_time = Time.now
