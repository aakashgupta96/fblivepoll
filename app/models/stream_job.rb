  ...
  def self.perform(post_id)
    ...
    if Rails.env.production?
      if source_live
        ...
        command = "$HOME/bin/ffmpeg -s 1280x720 -r 24 -f x11grab -i :#{headless.display}.0+0,66 -f alsa -ac 1 -i hw:0,1 -codec:a aac -ar 44100 -b:a 128k -preset ultrafast -vcodec libx264 -pix_fmt yuv420p -vb 2000k -r 24 -g 48 -f flv '#{@post.key}' 2>> #{Rails.root.join('log').join('stream').join(@post.id.to_s).to_s}"
      end
    else
      if source_live
