    ...
    if Rails.env.production?
      ...
      if source_live
        if Constant::RTMP_TEMPLATE_IDS.include?(@post.template.id)
          ...
          command = "$HOME/bin/ffmpeg -i '#{@post.get_file_url}' -codec:a aac -ac 1 -ar 48000 -b:a 128k -preset ultrafast -vcodec libx264 -pix_fmt yuv420p -vb 1500k -r 24 -g 48 -f tee -map 0:v -map 0:a '#{rtmp_keys}' 2>> #{Rails.root.join('log').join('stream').join(@post.id.to_s).to_s}"
        end
      else
        if Constant::RTMP_TEMPLATE_IDS.include?(@post.template.id)
          ...
          command = "$HOME/bin/ffmpeg -s 1280x720 -r 24 -f x11grab -i :#{headless.display}.0+0,66 -f alsa -ac 1 -i hw:0,1 -codec:a aac -ar 48000 -b:a 128k -preset ultrafast -vcodec libx264 -pix_fmt yuv420p -vb 1500k -r 24 -g 48 -f tee -map 0:v -map 1:a '#{rtmp_keys}' 2>> #{Rails.root.join('log').join('stream').join(@post.id.to_s).to_s}"
        end
      end
    else
      rtmp_keys = []
      ...
      if source_live
        ...
        if Constant::RTMP_TEMPLATE_IDS.include?(@post.template.id)
          ...
          command = "$HOME/bin/ffmpeg -s 1280x720 -r 24 -f x11grab -i :#{headless.display}.0+0,66 -i 'public/silent.aac' -ac 1 -codec:a aac -ar 48000 -b:a 128k -preset ultrafast -vcodec libx264 -pix_fmt yuv420p -vb 1500k -r 24 -g 48 -f tee -map 0:v -map 1:a '#{rtmp_keys}' 2> #{Rails.root.join('log').join('stream').join(@post.id.to_s).to_s}"
        end
      end
    end
    to_fix_alsa = true
